...
INSERT INTO sensors(ph, tds, turb, temp, flow, device_id, created_at, modified_at) VALUES(7.12, 80.43, 23.12, 28.43, 52.4, 1, ROUND(EXTRACT(EPOCH FROM NOW()) * 1000), ROUND(EXTRACT(EPOCH FROM NOW())) * 1000);
INSERT INTO sensors(ph, tds, turb, temp, flow, device_id, created_at, modified_at) VALUES(7.15, 80.49, 23.13, 28.49, 42.4, 1, ROUND(EXTRACT(EPOCH FROM NOW()) * 1000), ROUND(EXTRACT(EPOCH FROM NOW())) * 1000);
INSERT INTO sensors(ph, tds, turb, temp, flow, device_id, created_at, modified_at) VALUES(6.15, 85.49, 23.11, 27.79, 66.4, 2, ROUND(EXTRACT(EPOCH FROM NOW()) * 1000), ROUND(EXTRACT(EPOCH FROM NOW())) * 1000);